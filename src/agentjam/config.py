...
All filesystem state lives under AGENTJAM_HOME (default: ~/.agentjam).
"""
...
def _home() -> Path:
    """Return the agentjam home directory, respecting AGENTJAM_HOME env var."""
    if env := os.environ.get("AGENTJAM_HOME"):
        return Path(env)
    return Path.home() / ".agentjam"
