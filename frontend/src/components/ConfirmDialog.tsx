...
function ConfirmDialog({ open, title, message, confirmLabel = 'Confirm', danger, onConfirm, onCancel }: Props) {
  return (
    <Dialog open={open} onClose={onCancel} width={400}>
      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, color: 'var(--ink)' }}>{title}</div>
      <div style={{ fontSize: 13, color: 'var(--ink2)', lineHeight: 1.5, marginBottom: 20 }}>{message}</div>
